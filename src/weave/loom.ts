...
export interface LoomOptions {
  ...
  opening: Opening;
  /** Opt-in dimensional iceberg ordering (overview-first). Defaults from DevFlags.iceberg. */
  iceberg?: boolean;
}
...
// When one thread GROUNDS the other (a defining/foundational link).
const FOUNDATION_PROMPTS: ((a: string, b: string) => string)[] = [
  (a, b) => `${a} grounds ${b} — what falls apart if that foundation is wrong?`,
  (a, b) => `Why must you understand ${a} before ${b} can make sense?`,
];
// When one thread is PUT TO WORK through the other (an applied link).
const APPLICATION_PROMPTS: ((a: string, b: string) => string)[] = [
  (a, b) => `Where would you reach for ${a} and ${b} together in practice?`,
  (a, b) => `Walk through using ${a} to handle a real case of ${b}.`,
];
...
export class Loom {
  ...
  private flashcardConcepts = new Set<string>(); // concepts already flashcarded
  private sinceFlashcard = 0; // cards since the last recall flashcard
  ...
  // Opt-in dimensional iceberg ordering (off unless DevFlags.iceberg === true).
  private readonly iceberg: boolean;
  ...
  constructor(
    ...
    this.iceberg = this.opts.iceberg ?? loadSettings().dev?.iceberg === true;
  ...
  next(): FeedCard | null {
    ...
    const flashcard = this.maybeFlashcard();
  ...
  private maybeCheckpoint(): CheckpointCard | null {
    ...
    // Route the Socratic prompt by the rhetorical relation between the threads,
    // so the question fits the link (contrast / foundation / application / weave).
    const kind = threadKind(pb, docB, [best.a, best.b]);
    const contrast = kind === 'contrasts' || hasContrastCue(pa.text) || hasContrastCue(pb.text);
    const bank = contrast
      ? CONTRAST_PROMPTS
      : kind === 'applies'
        ? APPLICATION_PROMPTS
        : kind === 'defines' || kind === 'grounds'
          ? FOUNDATION_PROMPTS
          : WEAVE_PROMPTS;
  ...
  // -- flashcards (active recall, non-gate) ---------------------------------

  /** A recall flashcard for a warm, source-grounded concept not yet flashcarded.
   *  Non-gate: interleaves spaced recall without locking the feed. */
  private maybeFlashcard(): FlashcardCard | null {
    if (this.position < 3 || this.w.flashcardEvery <= 0) return null;
    if (this.sinceFlashcard < this.w.flashcardEvery) return null;
    for (const c of this.corpus.concepts) {
      if (!c.important || this.flashcardConcepts.has(c.id)) continue;
      if ((this.exposure.get(c.id) ?? 0) < this.w.hotExposure) continue;
      // only when its answer is genuinely grounded: a SEEN defining passage, or a definition.
      const grounded = (c.definedByPassage && this.emitted.has(c.definedByPassage)) || !!c.definition;
      if (!grounded) continue;
      const built = buildFlashcard(c, this.corpus.passages, this.corpus.docs);
      if (!built) continue;
      this.flashcardConcepts.add(c.id);
      this.sinceFlashcard = 0;
      this.lastKind = 'flashcard';
      const { id, index } = this.nextId();
      return {
        kind: 'flashcard',
        id,
        ...
        back: built.back,
        source: built.source,
        reason: `You've met “${built.label}” a few times — flip to check you can recall it from memory.`,
      };
    }
    return null;
  }
  ...
  /** Iceberg target: descends 5 -> 1 as mastery climbs, so the feed opens on the
   *  high-dimensional overview (D5 concepts / D3 forms) then dials into D2/D1 detail. */
  private dimTarget(): number {
    return 5 - 4 * Math.max(0, Math.min(1, this.stage().score));
  }
  ...
  private pickPassage(): PassageCard | null {
    ...
    const dimT = this.iceberg ? this.dimTarget() : 0; // descending overview->detail target
    ...
    for (let pass = 0; pass < 2; pass++) {
      ...
      for (const passage of candidates) {
        ...
        // Iceberg (opt-in): pull toward the descending dimensional target so the
        // session opens on the high-dimensional overview and dials into lower-D detail.
        if (this.iceberg) {
          const dims = concepts.map((c): number => c.dimension ?? 0).filter((d) => d > 0);
          if (dims.length) {
            const dimLevel = dims.reduce((s, d) => s + d, 0) / dims.length;
            score -= this.w.icebergWeight * Math.abs(dimLevel - dimT);
          }
        }
        if (
          pass === 1 &&
