...
import { fetchSenses, type Sense } from '../research/disambiguate';
...
import { annotateDimensionsLite } from '../weave/dimensions';
...
export type Phase = 'disambiguating' | 'researching' | 'expanding' | 'weaving' | 'ready' | 'empty';
...
export function useSession(query: string, presetCorpus?: Corpus) {
  ...
  const [senses, setSenses] = useState<Sense[]>([]); // pre-feed disambiguation choices ([] = unambiguous)
  const senseResolver = useRef<((q: string) => void) | null>(null); // resolves once the user picks a sense
  ...
  const expandMore = useCallback(async () => {
    ...
    try {
      ...
      annotateDimensionsLite(concepts, merged.passages);
      corpus.docs = docMap;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Pre-feed disambiguation: if the query is genuinely ambiguous, let the learner
      // pick the sense BEFORE we gather, so the feed isn't a mix of senses (or worse,
      // latched onto the wrong one). Fail-open: unambiguous queries return [] and skip
      // straight to research. The pipeline pauses here until chooseSense() resolves.
      let effectiveQuery = query;
      const found = await fetchSenses(query);
      if (cancelled) return;
      if (found.length >= 2) {
        setSenses(found);
        setPhase('disambiguating');
        effectiveQuery = await new Promise<string>((resolve) => {
          senseResolver.current = resolve;
        });
        senseResolver.current = null;
        if (cancelled) return;
        setSenses([]);
        setPhase('researching');
      }

      const seed = await research(effectiveQuery, onProgress);
      if (cancelled) return;
    ...
    return () => {
      cancelled = true;
      aliveRef.current = false;
      senseResolver.current?.(query); // unblock a pending disambiguation gate on teardown
      senseResolver.current = null;
    };
  ...
  // The learner picked a sense in the disambiguation gate -> resolve the paused
  // pipeline with the refined query (or the original, for "search as I typed it").
  const chooseSense = useCallback((refinedQuery: string) => {
    senseResolver.current?.(refinedQuery);
  }, []);
