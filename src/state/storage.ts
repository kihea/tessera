...
/** The user's whiteboard for a topic: idea-nodes they place and connections they
 *  draw -- their own synthesis, stored per topic alongside the notes. */
export interface WhiteboardData {
  nodes: { id: string; x: number; y: number; label: string }[];
  edges: { from: string; to: string }[];
}

export function loadWhiteboard(slug: string): WhiteboardData | null {
  return read<WhiteboardData>(`tessera:whiteboard:${slug}`);
}

export function saveWhiteboard(slug: string, data: WhiteboardData): void {
  write(`tessera:whiteboard:${slug}`, data);
}
...
export type ThemeName = 'standard' | 'dark' | 'light' | 'alexandria' | 'terminal' | 'fluid';
...
export const THEMES: { key: ThemeName; label: string; blurb: string }[] = [
  ...
  { key: 'dark', label: 'Dark', blurb: 'Clean modern dark — neutral, high-contrast, no flourishes.' },
  { key: 'light', label: 'Light', blurb: 'Clean modern light — crisp and bright for daytime reading.' },
  { key: 'fluid', label: 'Fluid', blurb: 'A living blue raymarch field beneath translucent glass.' },
...
export interface DevFlags {
  ...
  embeddings?: boolean;
  /** Dimensional ICEBERG feed ordering. OPT-IN (default OFF, unlike the flags above):
   *  open on the high-dimensional overview, then dial into D2/D1 detail. Read with
   *  `loadSettings().dev?.iceberg === true`, never devFlag() (which defaults on). */
  iceberg?: boolean;
}
